...
import (
	...
	"http2mcp/mcp-common/logger"
	"http2mcp/mcp-common/version"
	"http2mcp/mcp-manager/auth/jwt"
	"http2mcp/mcp-manager/database"
	managerHandler "http2mcp/mcp-manager/handler"
	"http2mcp/mcp-manager/i18n"
	"http2mcp/mcp-manager/middleware"
...
func initRouter(db database.Database, store storage.Store, ntf notifier.Notifier, cfg *config.APIServerConfig, logger *zap.Logger) *gin.Engine {
	...
	authH := managerHandler.NewHandler(db, jwtService, mcpCfg, logger)
	...
	{
		chatHandler := managerHandler.NewChat(db, logger)
		mcpHandler := managerHandler.NewMCP(db, store, ntf, logger)
		openapiHandler := managerHandler.NewOpenAPI(db, store, ntf, logger)
		systemPromptHandler := managerHandler.NewSystemPrompt(db, logger)
		...
		userMgmt.Use(managerHandler.AdminAuthMiddleware())
		{
		...
		tenantMgmt.Use(managerHandler.AdminAuthMiddleware())
		{
	...
	r.GET("/api/runtime-config", managerHandler.HandleRuntimeConfig)

	r.Static("/mcp-web", "./mcp-web")
	return r
}
